...
namespace SocketSever
{
    public class SocketManager
    {
        ...
        public void Send(IPEndPoint endpoint, string message)
        {
            ...
            if (argSend != null)
            {
                AsyncUserToken asyncUserToken = argSend.UserToken as AsyncUserToken;
                SocketAsyncEventArgs sendArg = new SocketAsyncEventArgs();
                sendArg.UserToken = asyncUserToken;
                ...
                bool willRaiseEvent = asyncUserToken.Socket.SendAsync(sendArg);  //The same socket Instance can not be reused in one proccess.
                if (!willRaiseEvent)
                {
                    ProcessSend(argSend);
                }
        ...
        public void SendToAllClient(string message)
        {
            ...
            foreach (var e in _connectedPool)
            {
                Send((e.UserToken as AsyncUserToken).EndPort, message);
            }
        }
